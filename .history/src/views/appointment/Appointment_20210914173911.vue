...
//查看当前阶段，查看用户是否报名
import { warning, getSessionValue, getObjValue, grtCurrentStatus, checkEnroll, CurrentStatus} from "../../utils/usualUse.js";
// 当前所处时段
let data = ref("...");
// 预约日期
let day = reactive([]);
...
onMounted(() => {
  ...
  if( !checkAllappointment() ) {
    getObjValue(close) = true;
    getObjValue(closeMessage) = `当前处于 ${currentStatus} 阶段，预约暂未开放，请耐心等候`;
    return;
  }
