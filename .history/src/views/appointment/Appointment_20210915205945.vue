...
//closeMessage
let closeMessage = ref("closeMessage");
//loading
...
onMounted(() => {
  ...
  checkEnroll({})
    .then((res) => {
      if (res.code === 700) {
        ...
        console.log(122345678);
        warning("您还未报名，请先进行报名");
        return;
      }
      //已报名
      isEnroll.value = true;
      // success(res.message + "appointment224");
    })
    .catch((err) => {
      warning(err.message + "appointment227");
    });
  ...
  if (isEnroll) {
    ...
    getCurrentStatu({}).then(res=>{
      ...
      warning(err.message + "appointment240");
    })
  }
  // //1、查看目前总体招新状态
  // if (getCurrentStatusId() === 1 || getCurrentStatusId() === 2) {
  //   console.log("处于报名状态");
  //   //当是1，2前两个阶段的时候，关闭预约功能
  //   close.value = true;
  //   closeMessage.value = "目前仍处于报名阶段，您已成功报名，请耐心等候";
  ...
  //   console.log("预约不能开饭");
  //   close.value = true;
  //   closeMessage.value = `当前处于 ${currentStatus} 阶段，预约暂未开放，请耐心等候`;
  //   return;
  // }
  ...
  //     });
  //   closeMessage.value = `您已成功预约，预约时间段为${totalTime}`;
  //   return;
  // }
