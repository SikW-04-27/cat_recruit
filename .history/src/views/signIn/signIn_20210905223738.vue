...
//是否报名
let hasSignUp = JSON.parse(window.localStorage.getItem("hasSignUp"));
//warning:
...
onMounted(() => {
  ...
  console.log(hasSingUp);

  //若处于前两个阶段
  if (JSON.parse(window.localStorage.getItem("CurrentStatus")).id === 1 || 2) {
    //处于前两阶段,即需要先用户报名
    console.log("chuyuqianliangzhuangtai");
    if (hasSignUp === true) {
      //若已经报名
      console.log("已经报名");
    } else {
  ...
  //     //未处于签到状态
  //     opening.value = false;
  //     closeMessage.value = "未开放签到，请耐心等候";
  //   }
  // });
  //获取用户是否已报名
  // getUserStatus({})
  //   .then((res) => {
  //     if (res.data.userStatus === "已报名" || "未预约") {
  //     } else {
  //       opening.value = false;
  //       closeMessage.value = "用户未报名，请先报名";
  //     }
