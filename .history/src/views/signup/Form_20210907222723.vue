...
if (localStorage.getItem("token")) {
  console.log(123);
  currentStatusId = JSON.parse(window.localStorage.getItem("CurrentStatus")).id;
  currentStatus = JSON.parse(window.localStorage.getItem("CurrentStatus"))
    .status;
  //判断是否报名
  isSignUp = JSON.parse(window.localStorage.getItem("hasSignUp"));
}
//获取学院
    ...
    if (currentStatusId === 2) {
      ...
      getSubmit({ uuid: stuId })
        .then((res) => {
          if(res.data === null){
            //未报名
            window.localStorage.setItem("hasSignUp",false) = false
          }
        })
        .catch((err) => {
          console.log(err);
